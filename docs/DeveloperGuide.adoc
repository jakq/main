...
Cons:
*** Does not follow the logic of the original Address Book 3 application. More code needs to be refactored.

* Alternative 2: Making Tag field final.
Pros:
*** Follows the logic of the original Address Book 3 application; don't need to refactor more code.
Cons:
...
Ste] 4. The user then decides to execute `edit review 1 ...` to edit the first review of the Eatery opened in the previous step. This calls the `EatMe#editcommand()`.
